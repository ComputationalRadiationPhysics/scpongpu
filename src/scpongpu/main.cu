...
int main(void)
{
  ...
  std::cout << "Number of particles: " << param::N_particle << std::endl;
  std::cout << "blocksize:           " << param::blocksize << std::endl;

  /* set up start parameters and copy them to GPU:*/
  vec location_h[param::N_particle];   /* particle location on host */
  vec speed_h[param::N_particle];      /* particle speed on host*/
  vec accel_h[param::N_particle];      /* particle acceleration on host*/

  /* TODO replace magic numbers */
  Uniformly<numtype, SeedSelected> uni(-1.0e-5f, 1.0e-5f);  /* random distribution for positioning */
  for(unsigned int i=0; i<param::N_particle; ++i)
    {
      /* give each particle a random position */
      location_h[i].x = uni.get() * param::chi_length;
      location_h[i].y = uni.get() * param::chi_length;
      location_h[i].z = uni.get() * param::chi_length;
  ...
  print_particles(location_h, speed_h, param::N_particle);

  /* prepare GPU part: */
  unsigned int size = sizeof(vec) * param::N_particle; /* size of one copy of all particles */
  ...
  dim3 gridDim(param::N_particle/param::blocksize, 1, 1);
  dim3 blockDim(param::blocksize, 1, 1);
